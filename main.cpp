...
extern "C" int wmain(int argc, wchar_t** argv)
{
    ...
    try {
        ...
        if (variablesMap.count("help") || (!variablesMap.count("file-name"))) {
            ...
            auto factoryLambda = [&variablesMap]() -> std::unique_ptr<BulkLoader> {
                ...
                if (variablesMap.count("host") && !variablesMap["host"].defaulted()
                    && !boost::iequals(host, boost::asio::ip::host_name()) && (host != "127.0.0.1"s) && !boost::iequals(host, "localhost"s)) {
            ...
            if (!variablesMap.count("dry-run")) {
                ...
                if (rejectedRecords.value_or(0) > 0) {
                    std::cout << "Server rejected " << rejectedRecords.value() << " records" << std::endl;
                }
            }
        }
